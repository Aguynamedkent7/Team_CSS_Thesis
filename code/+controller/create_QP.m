...
if isControlModelLinear
    if isfield(vhP, 'relax_terminal') && vhP.relax_terminal
...
if isControlModelLinear
    if isfield(vhP, 'relax_terminal') && vhP.relax_terminal
        % terminal velocity not forced to zero
    else
...
if isfield(vhP, 'Q_vel') && ~isControlModelLinear
    % Maximize speed ONLY at the terminal horizon state (exit speed) 
    % to act as a tie-breaker without causing physically impossible braking demands
    objective_lin(idx_x(vhP.Hp, 3)) = -vhP.Q_vel;
end

% Slip angle penalty
if isfield(vhP, 'W_alpha') && ~isControlModelLinear
    for k = 1:vhP.Hp
        v_long = X_opt_prev(3, k);
        v_lat  = X_opt_prev(4, k);
        dyaw   = X_opt_prev(6, k);
        ...
        z = (dyaw * vhModel.p.l_r - v_lat) / v_long;
        dz_dvlong = -z / v_long;
        dz_dvlat = -1 / v_long;
        dz_ddyaw = vhModel.p.l_r / v_long;
        
        dalpha_dz = 1 / (1 + z^2);
        
        grad_alpha = zeros(vhModel.nx, 1);
        grad_alpha(3) = dalpha_dz * dz_dvlong;
        grad_alpha(4) = dalpha_dz * dz_dvlat;
        grad_alpha(6) = dalpha_dz * dz_ddyaw;
        
        alpha_prev = atan(z);
        
        H_alpha = 2 * vhP.W_alpha * (grad_alpha * grad_alpha');
        objective_quad(idx_x(k,:), idx_x(k,:)) = objective_quad(idx_x(k,:), idx_x(k,:)) + H_alpha;
        
        f_alpha = 2 * vhP.W_alpha * alpha_prev * grad_alpha - H_alpha * X_opt_prev(:, k);
        objective_lin(idx_x(k,:)) = objective_lin(idx_x(k,:)) + f_alpha;
    end
end
...
% Minimize slack var
if isfield(vhP, 'dynamic_slack') && vhP.dynamic_slack
    max_kappa = 0;
    for k = 1:vhP.Hp-1
        v1 = checkpoints(checkpoint_indices(k)).forward_vector;
        v2 = checkpoints(checkpoint_indices(k+1)).forward_vector;
        ang = acos(max(min(dot(v1, v2) / (norm(v1)*norm(v2) + 1e-6), 1), -1));
        if ang > max_kappa
            max_kappa = ang;
        end
    end
    % Scale S with curvature to prevent washing out (drift)
    dynamic_S = vhP.S * (1 + vhP.slack_kappa_weight * max_kappa);
    objective_lin(idx_slack) = dynamic_S;
else
    objective_lin(idx_slack) = vhP.S;
    if isfield(vhP, 'strict_track_limits') && vhP.strict_track_limits
...
if ~isControlModelLinear
    ...
    if isfield(vhP, 'relax_terminal') && vhP.relax_terminal
        bound_upper(idx_x(end,3)) = 2.0; % Relaxed terminal speed
        bound_lower(idx_x(end,3)) = 0.0;
    else
