...
% Enhanced Phase 1 comparison
scenarios(42) = config.scenario_SL_vs_SCR_vs_Enhanced(scenarios(1));
...
if nargin == 0 % default: run only selected scenario
    output_file = sim.run(scenarios(42));
    ...
    for i_vehicle = 1:n_vehicles
        for i_lap = 1:n_laps
            ...
            fprintf('Vehicle %i (%s) Lap %i Time: %.2f seconds\n', i_vehicle, sim_result.cfg.scn.vhs{i_vehicle}.description, i_lap, t_lap);
        end
    end
    fprintf('--------------------\n');
